...
export default function EditTimesheetPage({
    ...
    async function loadTimesheet() {
      try {
        ...
        if (response.data.status !== "incomplete") {
          setError(
            "Only incomplete timesheets can be updated."
          );
          return;
        }
  ...
  async function handleSave() {
    ...
    if (entries.length === 0) {
      setError(
        "Please add at least one timesheet entry."
      );
      return;
    }
    ...
    try {
      ...
      setSuccess(
        response.data.status === "completed"
          ? "Timesheet completed successfully."
          : "Timesheet updated successfully."
      );
      showToast(response.data.status === "completed" ? "Timesheet completed successfully." : "Timesheet updated successfully.");
    } catch (error) {
      const message =
        error instanceof Error
          ? error.message
